...
<section class="border-b border-line">
    <div class="max-w-6xl mx-auto px-5 sm:px-8 py-6 sm:py-10">
        <h1 class="font-display font-medium text-3xl sm:text-5xl text-ink leading-none">我的内容</h1>
        <p class="font-display italic text-base sm:text-lg text-ink-2 mt-3">所有你写过的 8 大类内容贴。</p>
    </div>
</section>
...
<section class="sticky top-14 z-30 bg-paper/95 backdrop-blur-sm border-b border-line">
    <div class="max-w-6xl mx-auto px-5 sm:px-8 py-3">
        <div class="flex gap-2 overflow-x-auto scrollbar-hide">
            <a href="?filter=all" class="px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.15em] border {{ ($filter ?? 'all')==='all' ? 'bg-ink text-paper border-ink' : 'border-line-2 text-ink-2 hover:border-ink' }} whitespace-nowrap">全部</a>
...
<div class="max-w-6xl mx-auto px-5 sm:px-8 py-8">
    @forelse($contents as $i => $c)
        <a href="{{ url('/content/' . $c->id) }}" class="block border-b border-line py-4 group hover:bg-paper-2 transition-colors -mx-2 px-2">
            <div class="grid grid-cols-12 gap-3 items-baseline">
                <div class="col-span-2 sm:col-span-1 font-mono text-[10px] text-ink-3 uppercase tracking-[0.15em]">
                    {{ \App\Models\Content::TYPES[$c->type]['icon'] ?? 'N°00' }}
                </div>
                <div class="col-span-7 sm:col-span-7">
                    <h3 class="font-display text-lg text-ink group-hover:text-warm transition-colors leading-tight">{{ $c->title }}</h3>
                    <div class="font-mono text-[10px] text-ink-3 mt-1.5 uppercase tracking-[0.15em]">
                        {{ \App\Models\Content::TYPES[$c->type]['label'] ?? $c->type }} · {{ $c->places->first()?->city ?? '—' }} · {{ $c->created_at->diffForHumans() }}
                    </div>
                </div>
    ...
    @empty
        <div class="py-20 text-center">
            <div class="font-display text-2xl text-ink-2 mb-2">还没有内容贴</div>
            <a href="/" class="font-mono text-[11px] text-warm uppercase tracking-[0.2em]">去写一个 →</a>
        </div>
    @endforelse
</div>
